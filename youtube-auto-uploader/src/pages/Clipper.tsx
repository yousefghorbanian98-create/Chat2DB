...
import type { ClipRow, EditSuggestion } from '../global';
...
export function Clipper(): JSX.Element {
  ...
  const [speechStats,setSpeechStats]=useState<{wordCount:number;fillerCount:number;fillerRatio:number;wordsPerMinute:number}>();
  ...
  useEffect(() => {
    ...
    const statsUnsubscribe=window.api.on('speech:stats',value=>setSpeechStats(value as {wordCount:number;fillerCount:number;fillerRatio:number;wordsPerMinute:number}));
    const suggestionsUnsubscribe=window.api.on('edit:suggestions',()=>{void window.api.clipper.editSuggestions().then(setEditSuggestions)});
    ...
    return () => { progressUnsubscribe(); clipUnsubscribe(); speakerUnsubscribe(); faceUnsubscribe(); statsUnsubscribe(); suggestionsUnsubscribe(); pullUnsubscribe(); modelUnsubscribe(); doneUnsubscribe(); };
  }, [checkEngines, load]);
  ...
  const startFolder = async (): Promise<void> => {
    ...
    const handles=await window.api.clipper.startFolder({model,whisperModel,language,analysisMode,processingProfile:profile,previewOnly:true,userIntent,count,maxLength,category,aspect,captions,smartZoom,music,blurBackground});
  ...
  const start = async (): Promise<void> => {
    ...
    try {
      ...
      setRunning(true);
      const handle = await window.api.clipper.start({ url: url || undefined, localPath: file, model, whisperModel, language, analysisMode, processingProfile: profile, previewOnly: true, userIntent, count, maxLength, category, aspect, captions, smartZoom, music, blurBackground });
      activeJobId.current = handle.jobId;
  ...
  return <div className="editor-page">
    ...
    <div className="editor-workspace">
      <aside className="asset-panel">
        ...
        {activeAssetTab==='ai'&&<div className="tool-panel-content p-3 space-y-2"><div className="panel-heading !px-0"><span>AI tools</span><WandSparkles size={14}/></div>{(['fast','balanced','professional'] as const).map(name=><button key={name} className={`w-full text-left rounded-lg border p-3 ${profile===name?'border-violet-500 bg-violet-950/30':'border-[#292b38] bg-[#0d0e13]'}`} onClick={()=>applyProfile(name)}><b className="text-xs capitalize">{name}</b><span className="block text-[9px] text-zinc-500 mt-1">{name==='fast'?'Local highlights with minimum resource use':name==='balanced'?'Speech, scenes, captions and local refinement':'Speakers, faces, dynamic framing and full quality'}</span></button>)}{speechStats&&<div className="grid grid-cols-2 gap-2 mt-3 text-[9px]"><div className="rounded border border-[#292b38] p-2"><span className="text-zinc-500">Words</span><b className="block">{speechStats.wordCount}</b></div><div className="rounded border border-[#292b38] p-2"><span className="text-zinc-500">Pace</span><b className="block">{speechStats.wordsPerMinute} WPM</b></div><div className="rounded border border-[#292b38] p-2"><span className="text-zinc-500">Fillers</span><b className="block">{speechStats.fillerCount}</b></div><div className="rounded border border-[#292b38] p-2"><span className="text-zinc-500">Filler rate</span><b className="block">{Math.round(speechStats.fillerRatio*100)}%</b></div></div>}{editSuggestions.length>0&&<div className="mt-3 rounded-lg border border-amber-900/40 bg-amber-950/20 p-3 text-[9px]"><b>Reviewable edit suggestions</b><div className="flex justify-between mt-2 text-zinc-400"><span>Filler words</span><strong>{editSuggestions.filter(item=>item.type==='filler'&&item.status==='pending').length}</strong></div><div className="flex justify-between mt-1 text-zinc-400"><span>Long pauses</span><strong>{editSuggestions.filter(item=>item.type==='silence'&&item.status==='pending').length}</strong></div>{editSuggestions.filter(item=>item.status==='pending').slice(0,5).map(item=><div key={item.id} className="mt-2 border-t border-amber-900/30 pt-2"><span>{item.type==='filler'?`“${item.text}”`: `${(item.end_time-item.start_time).toFixed(1)}s pause`}</span><div className="flex gap-1 mt-1"><button className="btn !p-1 text-[8px]" onClick={()=>void window.api.clipper.decideEdit(item.id,'approved').then(()=>setEditSuggestions(current=>current.map(row=>row.id===item.id?{...row,status:'approved'}:row)))}>Approve</button><button className="btn !p-1 text-[8px]" onClick={()=>void window.api.clipper.decideEdit(item.id,'rejected').then(()=>setEditSuggestions(current=>current.map(row=>row.id===item.id?{...row,status:'rejected'}:row)))}>Keep</button></div></div>)}{editSuggestions.some(item=>item.status!=='pending')&&<div className="mt-3 border-t border-amber-900/30 pt-2"><span className="text-zinc-500">Recent decisions</span>{editSuggestions.filter(item=>item.status!=='pending').slice(0,3).map(item=><div key={item.id} className="flex justify-between mt-1"><span>{item.status} · {item.type}</span><button className="text-violet-300" onClick={()=>void window.api.clipper.decideEdit(item.id,'pending').then(()=>setEditSuggestions(current=>current.map(row=>row.id===item.id?{...row,status:'pending'}:row)))}>Undo</button></div>)}</div>}</div>}</div>}
