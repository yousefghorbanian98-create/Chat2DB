...
a = Analysis(
    ['easyclip_engine.py'],
    pathex=[],
    binaries=ct_bins + fw_bins + pa_bins + cv_bins,
    datas=ct_datas + fw_datas + pa_datas + cv_datas,
    hiddenimports=ct_hidden + fw_hidden + pa_hidden + cv_hidden + ['yt_dlp', 'torch', 'torchaudio', 'cv2'],
    hookspath=[],
    runtime_hooks=[],
    excludes=[],
    noarchive=False,
