...
class TestConnectionPragmas:
    """Guards the durability/concurrency PRAGMAs (docs/MuscleParadise/build/02 §1)."""

    def test_busy_timeout_is_set(self, engine) -> None:
        # 0 means "fail instantly on contention" — the kiosk-vs-till bug.
        with engine.connect() as conn:
            assert conn.execute(text("PRAGMA busy_timeout")).scalar() >= 5000

    def test_synchronous_is_normal(self, engine) -> None:
        with engine.connect() as conn:
