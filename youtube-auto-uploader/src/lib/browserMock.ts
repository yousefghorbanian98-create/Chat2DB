...
export function installBrowserMock(): void {
  ...
  window.api = {
    ...
    clipper: { start: async () => handle(), startFolder: async () => [handle(),handle()], renderSuggested: async () => handle(), cancel: async () => true, job: async () => null, clips: async () => clips, editSuggestions: async () => [], decideEdit: async () => undefined, update: async () => undefined, export: async () => null, exportGif: async () => null, upload: async () => handle() },
    ...
    updates: { state: async () => ({status:'idle',currentVersion:'2.0.0',canAutoInstall:true}), check: async () => ({status:'not-available',currentVersion:'2.0.0',canAutoInstall:true}), download: async () => ({status:'downloaded',currentVersion:'2.0.0',availableVersion:'1.2.0',percent:100,canAutoInstall:true}), install: async () => undefined },
    ...
    huggingFace: { state: async () => ({ configured: false, verified: false, repositories: [] }), save: async () => ({ configured: true, verified: true, username: 'preview', repositories: [{ id: 'pyannote/speaker-diarization-3.1', accessible: true }, { id: 'pyannote/segmentation-3.0', accessible: true }] }), remove: async () => ({ configured: false, verified: false, repositories: [] }) },
