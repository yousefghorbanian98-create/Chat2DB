...
export function Clipper(): JSX.Element {
  ...
  const [editSuggestions,setEditSuggestions]=useState<{fillers:Array<{text:string;start:number;end:number;confidence:number;reason:string}>;silences:Array<{start:number;end:number;confidence:number;reason:string}>}>();
  ...
  useEffect(() => {
    ...
    const suggestionsUnsubscribe=window.api.on('edit:suggestions',value=>setEditSuggestions(value as {fillers:Array<{text:string;start:number;end:number;confidence:number;reason:string}>;silences:Array<{start:number;end:number;confidence:number;reason:string}>}));
  ...
  return <div className="editor-page">
    ...
    <div className="editor-workspace">
      <aside className="asset-panel">
        ...
        {activeAssetTab==='ai'&&<div className="tool-panel-content p-3 space-y-2"><div className="panel-heading !px-0"><span>AI tools</span><WandSparkles size={14}/></div>{(['fast','balanced','professional'] as const).map(name=><button key={name} className={`w-full text-left rounded-lg border p-3 ${profile===name?'border-violet-500 bg-violet-950/30':'border-[#292b38] bg-[#0d0e13]'}`} onClick={()=>applyProfile(name)}><b className="text-xs capitalize">{name}</b><span className="block text-[9px] text-zinc-500 mt-1">{name==='fast'?'Local highlights with minimum resource use':name==='balanced'?'Speech, scenes, captions and local refinement':'Speakers, faces, dynamic framing and full quality'}</span></button>)}{speechStats&&<div className="grid grid-cols-2 gap-2 mt-3 text-[9px]"><div className="rounded border border-[#292b38] p-2"><span className="text-zinc-500">Words</span><b className="block">{speechStats.wordCount}</b></div><div className="rounded border border-[#292b38] p-2"><span className="text-zinc-500">Pace</span><b className="block">{speechStats.wordsPerMinute} WPM</b></div><div className="rounded border border-[#292b38] p-2"><span className="text-zinc-500">Fillers</span><b className="block">{speechStats.fillerCount}</b></div><div className="rounded border border-[#292b38] p-2"><span className="text-zinc-500">Filler rate</span><b className="block">{Math.round(speechStats.fillerRatio*100)}%</b></div></div>}{editSuggestions&&<div className="mt-3 rounded-lg border border-amber-900/40 bg-amber-950/20 p-3 text-[9px]"><b>Reviewable edit suggestions</b><div className="flex justify-between mt-2 text-zinc-400"><span>Filler words</span><strong>{editSuggestions.fillers.length}</strong></div><div className="flex justify-between mt-1 text-zinc-400"><span>Long pauses</span><strong>{editSuggestions.silences.length}</strong></div><p className="mt-2 text-zinc-500">Suggestions are never removed before review.</p></div>}</div>}
