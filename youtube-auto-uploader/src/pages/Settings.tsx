...
export function Settings(): JSX.Element {
  ...
  return <div className="page"><div className="flex items-end justify-between"><div><span className="text-[10px] tracking-[.18em] text-violet-400">VIOLETCUT SYSTEM</span><h1 className="text-2xl font-bold mt-1">Settings</h1><p className="muted text-sm mt-1">Hardware, publishing, and private model access.</p></div><ShieldCheck className="text-violet-400" size={30}/></div><div className="grid grid-cols-2 gap-5 mt-6">
    ...
    <Section title="Account and diagnostics"><button className="btn btn-primary" onClick={()=>void window.api.diagnostics.export().then(file=>file&&toast.success(`Diagnostic report saved to ${file}`))}>Export privacy-safe diagnostic report</button><p className="text-xs muted">Tokens, home paths, application data paths, and web addresses are removed from the report.</p><button className="btn" onClick={()=>void window.api.auth.logout().then(()=>location.reload())}>Sign out</button><Toggle label="Enable production developer tools" value={settings.devtools} onChange={(value)=>void save({devtools:value})}/></Section>
