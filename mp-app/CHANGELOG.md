...
## [Unreleased] — 2026-09-06 · engineering audit

### Added
- `.github/workflows/mp-ci.yml` — the quality gate the project never had: pytest
  (coverage ≥85%), `npm run gate`, an OpenAPI drift check, a real-HTTP smoke job
  asserting member isolation (403) and bad-PIN rejection (401), a `clock-safety`
  job running the suite from `Pacific/Kiritimati` and `Pacific/Midway`, and an
  advisory security scan (pip-audit, bandit, npm audit).
- `docs/MuscleParadise/build/` — 13-document zero-to-one build book: production
  requirements, DB design, API design, frontend, backend, security, DevOps,
  testing, design system, OSS toolchain, audit, roadmap.
- Two schema tests pinning the new connection PRAGMAs.

### Fixed
- `useWorkoutLog.test.ts` asserted a hard-coded `2026-08-30` session date, so the
  suite had been failing on every day after that. Clock frozen with fake timers;
  the expected value is now derived from `todayIso()` so it holds in every
  timezone (verified in UTC, Kiritimati, Midway, Tehran).
- `openapi.yaml` was stale by **7 routes** — the real surface is **49**, not 42.
  Four of the missing ones were athlete-facing (`/client/me/checkin-qr`,
  `/client/me/injuries`, `/client/me/payments`, `/client/me/workouts`).
  Regenerated, and CI now fails on drift.
- `app/db.py`: added `PRAGMA busy_timeout = 5000` and `synchronous = NORMAL`.
  Without the timeout, a kiosk scan concurrent with a till write returned an
  immediate `SQLITE_BUSY` (HTTP 500) instead of waiting.

### Measured
- Backend: **264 passed**, coverage **90.92%** (gate 85%).
