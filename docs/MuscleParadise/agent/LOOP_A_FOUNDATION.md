# LOOP A — پایه و امنیت
## ۲۰ تسک · ~۲ هفته · **مسدودکننده: هیچ لوپ دیگری قبل از اتمام این شروع نمی‌شود**
...
## A-20 · ممیزی منابع اوپن‌سورس درخواستی کاربر
| **فایل‌ها** | `docs/MuscleParadise/build/10_OSS_TOOLCHAIN.md` · `mp-app/NOTICES.md` |
|---|---|
کاربر چهار مخزن را برای بررسی معرفی کرد. نتیجهٔ ممیزی **ثبت شود** تا دوباره بررسی نشود:

| مخزن | چیست | مجوز | حکم |
|---|---|---|---|
| `1jehuang/jcode` | هارنس عامل کدنویسی (Rust، ۱۹.۱k ⭐) | MIT | ✅ **ابزار ساخت** — بیرون از محصول |
| `thiientv/godmode` | مهارت‌های عامل کدنویسی (۹۴ ⭐) | MIT | ✅ **ابزار ساخت** — بیرون از محصول |
| `arbazkhan971/godmode` | افزونهٔ عامل، ۱۲۶ مهارت (۲۶ ⭐) | MIT | ✅ **ابزار ساخت** — بیرون از محصول |
| `elder-plinius/G0DM0D3` | چت چندمدلی (۱۱k ⭐) | 🔴 **AGPL-3.0** | ❌ **ممنوع** |

🔴 **چرا G0DM0D3 ممنوع است:** AGPL-3.0 حتی از GPL سخت‌گیرانه‌تر است — اگر کدش
وارد محصول شود، باید **کل سورس Muscle Paradise منتشر شود**، حتی اگر فقط
سرویس بدهید. این قید C11 را می‌شکند و محصول قابل‌فروش را از بین می‌برد.

⚠️ **سه مورد اول ابزار *کدنویسی*اند، نه مغز محصول.** به عامل AI کمک می‌کنند
کد بنویسد؛ ربطی به تحلیل پیکرسنجی یا ساخت برنامهٔ تمرینی ندارند. در
`10_OSS_TOOLCHAIN.md` به‌عنوان **لایهٔ A** (ابزار توسعه) ثبت شوند.

🟢 یک ایدهٔ خوب در G0DM0D3 هست که *مفهومش* را می‌شود گرفت (بدون کد):
«مسابقهٔ موازی چند مدل و انتخاب بهترین». این دقیقاً `race()` موجود است —
که بهتر است چون داور ما معیار ورزشی دارد، نه امتیاز عمومی.

**verify:** `grep -c "AGPL" mp-app/NOTICES.md` = ۰ و هر سه ابزار در سند ثبت شده باشند

# معیار خروج LOOP A

```
□ ۲۰ تسک DONE (یا BLOCKED با دلیل مکتوب)
